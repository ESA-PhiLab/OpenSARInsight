...
Customer: ESA
---------------------------------------------------------------------
File: train_rfi_small_scratch.py

Description:
    This is the main training code used for RFI small model training from scratch using a Lightweight U-Net on RC data.

CLI
---
python train_rfi_small_scratch.py --config /path/to/config.yaml [--transforms /path/to/transforms.yaml] [--device cuda|cpu]
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train a small Lightweight U-Net for RFI segmentation from scratch."
    )
...
def main():
    ...
    eff_batch_target = 8 
    accumulate_steps = max(1, eff_batch_target // train_batch_size)  # 8//2 = 4
