"""
Vessel dataset train/val/test splitter

split_root/
  ├─ train/
  │    ├─ labels/20250520_xml/*.xml
  │    ├─ patches/*
  │    ├─ patches0/*
  │    └─ raw/*.dat
  ├─ val/
...
How to run this code: python split_vessel.py
...
# ====================== PATHS ======================
LABEL_DIR    = Path(cfg["split_vessel"]["label_dir"])
PATCHES_DIR  = Path(cfg["split_vessel"]["patches_dir"])
PATCHES0_DIR = Path(cfg["split_vessel"]["patches0_dir"])
RAW_DIR      = Path(cfg["split_vessel"]["raw_dir"])
OUTPUT_ROOT  = Path(cfg["split_vessel"]["output_root"])
...
def ensure_dirs(base: Path) -> tuple[Path, Path, Path, Path]:
    """Create split subdirs and return destination roots (dst_xml, dst_pt, dst_pt0, dst_rw)."""
    dst_xml = base / "labels" / "20250520_xml"
    dst_pt  = base / "patches"
    dst_pt0  = base / "patches0"
    dst_rw  = base / "raw"
    dst_xml.mkdir(parents=True, exist_ok=True)
    dst_pt.mkdir(parents=True, exist_ok=True)
    dst_pt0.mkdir(parents=True, exist_ok=True)
    dst_rw.mkdir(parents=True, exist_ok=True)
    return dst_xml, dst_pt, dst_pt0, dst_rw
...
def job_iter(
    ...
    scene_to_patches0: Dict[str, List[Path]],
    ...
    for split, scene_list in split_scenes.items():
        base = out_root / split
        dst_xml, dst_pt, dst_pt0, dst_rw = ensure_dirs(base)
        ...
        for scene in scene_list:
            ...
            for f in scene_to_patches0.get(scene, ()):
                yield (f, dst_pt0 / f.name,  hardlink, symlink)
...
def run() -> None:
    ...
    # Validate inputs
    for p in (LABEL_DIR, PATCHES_DIR, PATCHES0_DIR, RAW_DIR):
        if not p.is_dir():
            sys.exit(f"[ERROR] folder not found: {p}")
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)

    # LABEL_DIR must be the 20250520_xml folder (your structure already has labels/20250520_xml on input)
    if LABEL_DIR.name != "20250520_xml":
        sys.exit(f"[ERROR] LABEL_DIR must be the 20250520_xml folder (got: {LABEL_DIR})")

    # 1) index
    xml_map   = index_folder(LABEL_DIR, (".xml",))
    patch_map = index_folder(PATCHES_DIR, IMG_EXTS, PATCH_TOKS)
    patch0_map = index_folder(PATCHES0_DIR, IMG_EXTS, PATCH_TOKS)
    ...
    scene_to_patches0 = remap_by_scene(patch0_map, id_to_scene)
    ...
    jobs = job_iter(split_scenes, scene_to_xmls, scene_to_patches, scene_to_patches0,  scene_to_raw, OUTPUT_ROOT, USE_HARDLINK, USE_SYMLINK)
    chunksize = 512
