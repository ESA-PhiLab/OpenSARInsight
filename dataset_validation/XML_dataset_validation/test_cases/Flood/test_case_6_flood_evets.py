DESCRIPTION = """
Test Case: Flood Events Validation for Flood
...
Inputs:
  ...
  - Ranges and coditions that need to be checked for each fields within the 'ProcessingData/List_of_FloodEvents/Event/Polygon':
...
def parse_float_list_with_nan(text, tag_name, min_val, max_val, errors):
    ...
    for v in raw_values:
        if v.strip().lower() == 'nan':
            parsed.append(float('nan'))  # Count it but don't validate range
            continue
        try:
            fval = float(v)
        except:
...
def validate_polygon(polygon_elem, errors, event_index):
    ...
    for a, b in coord_pairs:
        if a in lengths and b in lengths and lengths[a] != lengths[b]:
            errors.append(f"{a} and {b} have different lengths for Event #{event_index}: {lengths[a]} vs {lengths[b]}.")
...
def run_test(xml_root):
    ...
    number_text = xml_root.findtext('ProcessingData/StatisticsReport/Number_of_FloodEvents')
    try:
        num_events = int(number_text.strip())
    except:
        return {'status': 'FAIL', 'details': {'missing': ['Number_of_FloodEvents is not a valid integer.']}}

    if num_events == 0:
        return {'status': 'PASS', 'details': {'note': 'No flood events to validate.'}}

    events = xml_root.findall('ProcessingData/List_of_FloodEvents/Event')
    if not events:
        return {'status': 'FAIL', 'details': {'missing': ['Expected Events under ProcessingData/List_of_FloodEvents but none found.']}}

    for idx, event in enumerate(events, 1):
        polygon = event.find('Polygon')
        if polygon is None:
            missing_or_invalid.append(f"Missing Polygon in Event #{idx}.")
            continue
