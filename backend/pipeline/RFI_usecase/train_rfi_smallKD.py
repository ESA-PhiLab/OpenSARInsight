...
CLI
---
python train_rfi_smallKD.py --config /path/to/config.yaml [--transforms /path/to/transforms.yaml] [--device cuda|cpu]

History:
  - 2025-10-16: 
        KD + wandb
  - 2025-10-20: 
        KD refactor + CLI args
...
def main():
    args = parse_args()
    default_config_path = Path(__file__).parent.joinpath("config-rfi-kd.yaml")
    if not default_config_path.exists():
        default_config_path = os.environ.get("LRFIM_CONFIG_PATH")

    # Load configs/paths
    cfg = load_yaml_config(args.config) if args.config else load_yaml_config(default_config_path)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if ((epoch + 1) % max(1, eval_every)) == 0:
            ...
            LOGGER.info(
                "Epoch %3d/%3d | Train@val_thr: loss=%.5f dice=%.4f iou=%.4f acc=%.4f precision=%.4f recall=%.4f | thr=%.3f",
                epoch + 1, epochs,
                train_eval.loss, train_eval.dice, train_eval.iou, train_eval.acc,
