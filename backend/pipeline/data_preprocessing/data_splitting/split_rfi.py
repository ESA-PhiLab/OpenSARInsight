"""
RFI dataset train/val/test splitter

Output layout:
split/
  ├─ train/
  │    ├─ labels/xml_labelling/*.xml
  │    ├─ labels/binary_masks/*.png
  │    ├─ patches/*     # typically names contain _VV,_VH,_SLC,_GRD
  │    └─ raw/*.dat
  ├─ val/
  │    ├─ labels/xml_labelling/*.xml
  │    ├─ labels/binary_masks/*.png
...
How to run this code: python split_rfi.py
...
# ====================== PATHS ======================
LABEL_DIR: Path    = Path(cfg["split_rfi"]["label_dir"])
MASKS_DIR: Path    = Path(cfg["split_rfi"]["masks_dir"])
PATCHES_DIR: Path  = Path(cfg["split_rfi"]["patches_dir"])
RAW_DIR: Path      = Path(cfg["split_rfi"]["raw_dir"])
OUTPUT_ROOT: Path  = Path(cfg["split_rfi"]["output_root"])
...
USE_SYMLINK: bool   = False             # set True to allow symlinks across filesystems
VERBOSITY: int      = 1                 # 0=WARNING, 1=INFO, and 2=DEBUG
# ==========================================================================

RAW_EXTS: tuple[str, ...]   = (".dat",)
IMG_EXTS: tuple[str, ...]   = (".tif", ".tiff", ".png")
PATCH_TOKS: tuple[str, ...] = ("VV", "VH", "SLC", "GRD")
...
def ensure_dirs(base: Path) -> tuple[Path, Path, Path, Path]:
    """Create split subdirs and return destination roots (dst_xml, dst_mask, dst_pt, dst_rw)."""
    dst_xml = base / "labels" / "xml_labelling"
    dst_mask = base / "labels" / "binary_masks"
    dst_pt = base / "patches"
    dst_rw = base / "raw"
    ...
    return dst_xml, dst_mask, dst_pt, dst_rw
...
def job_iter(
    ...
    scene_to_masks: Dict[str, List[Path]],
    ...
    for split, scene_list in split_scenes.items():
        base = out_root / split
        dst_xml, dst_mask, dst_pt, dst_rw = ensure_dirs(base)

        for scene in scene_list:
            for f in scene_to_xmls.get(scene, ()):
                yield (f, dst_xml / f.name, hardlink, symlink)
            for f in scene_to_masks.get(scene, ()):
                yield (f, dst_mask / f.name, hardlink, symlink)
...
def run() -> None:
    ...
    # Enforce expected leaf names to catch mis-pointed roots
    if LABEL_DIR.name != "xml_labelling":
        sys.exit(f"[ERROR] LABEL_DIR must be the xml_labelling folder (got: {LABEL_DIR})")
    if MASKS_DIR.name != "binary_masks":
        sys.exit(f"[ERROR] MASKS_DIR must be the binary_masks folder (got: {MASKS_DIR})")
    ...
    scene_to_masks   = remap_by_scene(mask_map,  id_to_scene)
    ...
    logging.info(
        "Scenes %d → train %d, val %d, test %d",
        n, len(split_scenes["train"]), len(split_scenes["val"]), len(split_scenes["test"])
    )
