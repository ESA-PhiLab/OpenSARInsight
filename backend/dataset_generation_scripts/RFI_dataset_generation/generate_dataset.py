...
def expand_scene_entries(entries: Any) -> list[int] | None:
    """
    Same semantics as RFI run.py: entries can be ints or inclusive ranges "a-b".
...
def get_input_root(cfg: dict) -> Path:
    """Get input_root (different naming from DVD pipeline)."""
    p = cfg.get("input", {}).get("input_root")
    if not p:
        raise ValueError("Missing input.input_root in config")
    root = Path(p)
    if not root.exists():
        raise NotADirectoryError(f"input_root does not exist: {root}")
...
def get_mapping_csv(cfg: dict) -> Path:
    """Get mapping_csv path (different naming from DVD pipeline)."""
...
def build_slc_safe_path(input_root: Path, slc_name: str, slc_dirname: str = "slc") -> Path:
    slc_dir = input_root / slc_dirname
    name = normalize_safe_name(slc_name)
    return slc_dir / (name + ".SAFE")


def build_l0_decoded_path(input_root: Path, raw_name: str) -> Path:
    name = normalize_safe_name(raw_name)
    return input_root / "raw" / (name + ".SAFE")
...
def step1_rfi(scene_num: int, cfg: dict, out_root: Path, rfi_root: Path, mapping_csv: Path) -> Path:
    """
    Runs RFI_Codes_Python/run.py for a single scene by generating a per-scene config.
    Produces: scene_XXX/{slc,label,...}
    """
    print(f"\n  [STEP 1] RFI Detection Processing")
    print(f"  ==================================")
    ...
    # Ensure mapping_csv is in the temp config
    cfg2["input"]["mapping_csv"] = str(mapping_csv)
    tmp_cfg.write_text(yaml.safe_dump(cfg2, sort_keys=False))
    print(f"  [EXEC] Temp config written to: {tmp_cfg}")

    print(f"  [EXEC] Running RFI detection processing...")
    cmd = ["python", "run.py", "--config", str(tmp_cfg)]
    run_logged(cmd, log_path=log, cwd=rfi_root)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate RFI dataset pipeline: RFI → SLC2RAW → L0Patcher",
...
Examples:
  ...
  python generate_dataset.py --config /path/to/config.yaml
    ...
    print("="*70)
    print("RFI DATASET GENERATION PIPELINE")
    print("="*70)
    ...
    # Get subdirectory names (with defaults matching your run.py)
    slc_dirname = cfg.get("input", {}).get("slc_dirname", "slc")
    ...
    # FIX: Use semicolon delimiter and handle potential encoding issues
    ...
    try:
        with mapping_csv.open(newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                row_count += 1
                # Verify 'num' column exists
                if "num" not in row:
                    available_cols = list(row.keys())
                    raise KeyError(
                        f"Column 'num' not found in CSV. "
                        f"Available columns: {available_cols}. "
                        f"Check that the delimiter is ';' and file encoding is UTF-8."
                    )
                try:
                    num = int(row["num"])
                    mapping_by_num[num] = row
                except ValueError as e:
                    print(f"[WARN] Skipping row {row_count}: invalid 'num' value '{row.get('num')}': {e}")
                    continue
    except UnicodeDecodeError:
        # Fallback to latin-1 encoding
        print(f"[WARN] UTF-8 decode failed, trying latin-1 encoding...")
        with mapping_csv.open(newline="", encoding="latin-1") as f:
            reader = csv.DictReader(f, delimiter=";")
            row_count = 0
            for row in reader:
                row_count += 1
    ...
    for idx, scene_num in enumerate(wanted_scenes, 1):
        ...
        # RFI pipeline may not use raw, check if it exists in mapping
        ...
        slc_safe = build_slc_safe_path(input_root, slc_name, slc_dirname)
        ...
        # Only validate raw if needed for step 3
        l0_decoded = None
        if raw_name:
            l0_decoded = build_l0_decoded_path(input_root, raw_name)
            if not l0_decoded.exists():
                print(f"[WARN] ✗ L0 decoded not found: {l0_decoded}")
                print(f"[WARN] Step 3 (L0Patcher) will be skipped")
                l0_decoded = None
            else:
        ...
        # Step 1: RFI detection processing
        try:
            scene_out = step1_rfi(scene_num, cfg, out_root, script_dir, mapping_csv)
        except subprocess.CalledProcessError as e:
        ...
        # Label folder should be created by RFI processing
        label_folder = scene_out / "label"
        print(f"\n[CHECK] Verifying RFI outputs...")
        ...
        # Step 3: L0 patching (only if L0 data available)
        if l0_decoded:
            try:
            ...
            print(f"\n[SUCCESS] Scene {scene_num:03d} completed (steps 1-2 only)!")
            print(f"[INFO] Step 3 skipped (no L0 decoded data)")
